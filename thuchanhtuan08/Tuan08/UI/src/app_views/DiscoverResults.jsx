...
export default function DiscoverResults() {
  ...
  const filteredItemCatalog = query.trim()
    ? allItemCatalog.filter(r => r.title.toLowerCase().includes(query.trim().toLowerCase()))
    : allItemCatalog;
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between mb-8">
            <h1 className="text-3xl font-extrabold text-gray-900">
              {query ? (query.charAt(0).toUpperCase() + query.slice(1)) : 'All ItemCatalog'}{' '}
              <span className="text-gray-400 font-semibold text-xl">({filteredItemCatalog.length})</span>
            </h1>
          ...
          {filteredItemCatalog.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-10 mb-12">
              {filteredItemCatalog.map(recipe => (
