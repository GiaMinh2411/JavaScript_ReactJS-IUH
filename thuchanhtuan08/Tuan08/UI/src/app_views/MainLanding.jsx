...
import { useSession } from '../app_state/UserSession';
import { Link } from 'react-router-dom';

// ── Data ─────────────────────────────────────────────────────────────────────

const summerItemCatalog = [
...
function SectionTopBar({ title, subtitle }) {
  return (
    <div className="mb-8">
...
export default function MainLanding() {
  const { isLoggedIn, setShowSignInForm } = useSession();
  ...
  return (
    <div className="w-full">
        ...
        <section className="w-full rounded-3xl my-6 overflow-hidden relative shadow-md min-h-[450px] flex flex-col items-center justify-center text-center group cursor-pointer">
          ...
          <div className="relative z-20 px-6 py-12 flex flex-col items-center">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4">
              <button onClick={() => setShowSignInForm(true)} className="px-8 py-3.5 bg-pink-500 text-white rounded-full font-bold text-lg hover:bg-pink-600 transition shadow-lg shadow-pink-500/30">
      ...
      <div className="py-8 px-6 space-y-16">
        ...
        {/* 1 ── This Summer ItemCatalog — 4-column grid */}
        <section>
          <SectionTopBar
            title="This Summer ItemCatalog"
            subtitle="We have all your Independence Day sweets covered."
          />
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {summerItemCatalog.map(r => <StandardCard key={r.id} recipe={r} />)}
          </div>
        </section>

        {/* 2 ── ItemCatalog With Videos — 4-column grid with play button */}
        <section>
          <SectionTopBar
            title="ItemCatalog With Videos"
            subtitle="Cooking Up Culinary Creations with Step-by-Step Videos"
          />
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {videoItemCatalog.map(r => <VideoCard key={r.id} recipe={r} />)}
          </div>
        </section>
